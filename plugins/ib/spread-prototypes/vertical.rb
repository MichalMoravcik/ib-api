  ...
  module Vertical
    ...
    class << self
      ...
      def fabricate(master, buy: 0, sell: 0)
        error 'Argument must be an option' unless %i[option futures_option].include? master.sec_type
      ...
      def build from:, **fields
        underlying = if from.is_a? IB::Option
                       ...
                       unless fields.key?(:trading_class) || from.trading_class.empty?
                         fields[:trading_class] =
                           from.trading_class
                       end
                       unless fields.key?(:multiplier) || from.multiplier.to_i.zero?
                         fields[:multiplier] =
                           from.multiplier
                       end
      ...
      def the_description(spread)
        ...
        "<Vertical #{spread.symbol} #{spread.legs.first.right}(#{x.map do |w, strike|
          "#{w} :#{strike} "
        end.join('|+|')} )[#{date_str}]>"
      end
    end # class
