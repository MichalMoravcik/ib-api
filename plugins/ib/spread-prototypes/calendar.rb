  ...
  module Calendar
    ...
    class << self
      ...
      def fabricate(master, the_other_expiry)
        error 'Argument must be a IB::Future or IB::Option' unless %i[option future_option
                                                                      future].include? master.sec_type
      ...
      def build from:, front: nil, back: nil, right: :put, strike: nil, **fields
        ...
        initialize_spread(underlying) do |the_spread|
          ...
          leg2 = leg_prototype.merge(expiry: kind[:back]).verify.first unless leg2.is_a? IB::Option
          ...
          the_spread.description = begin
            the_description(the_spread)
      ...
      def the_description(spread)
        ...
        "<Calendar #{spread.symbol} #{f_or_o} [#{date_strs.join('|+|')}]>"
      end
    end # class
  end # module vertical
end # module ib
