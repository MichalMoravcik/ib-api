...
# Load socket stub for tests after ib-api is loaded
if ENV['TEST_ENV'] != 'real'
  require_relative 'support/socket_patch'
  IB::TestSocketPatch.apply!
end

require 'pp'
require 'yaml'

# Load test mocks after everything else is loaded
Dir.glob(File.expand_path('support/mocks/*.rb', __dir__)).sort.each { |f| require f }
...
RSpec.configure do |config|
  ...
  config.include IB::Test::FactoryHelper
end
