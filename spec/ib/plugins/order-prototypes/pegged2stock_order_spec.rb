...
RSpec.describe IB::Order do
  ...
  context 'Pegged to Stock order to dynamically sell a put  option on IBM' do
    ...
    When(:order) do
      IB::Pegged2Stock.order size: size, delta: delta, starting_price: starting_price,
    ...
    context 'Pegged orders are submitted as daily orders' do
      Then do
        order.serialize_extended_order_fields == ['DAY', nil, ACCOUNT, 'O', 0, nil, true, 0, false, false, nil, 0, false,
                                                  false]
      end
    end
